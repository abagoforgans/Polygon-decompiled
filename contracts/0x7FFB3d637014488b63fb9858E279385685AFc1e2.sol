...
function changeName(string arg1) {
    ...
    if this.address != msg.sender:
        if roleAdmin[0][1][address(msg.sender)].field_0:
            ...
            domainSeperator = sha3(sha3(0x73454950373132446f6d61696e28737472696e67206e616d652c737472696e672076657273696f6e2c6164647265737320766572696679696e67436f6e74726163742c627974657333322073616c74), sha3(arg1[all]), sha3('1'), address(this.address), chainid)
    ...
    mem[96] = calldata.size
    mem[128 len calldata.size] = call.data[0 len calldata.size]
    if roleAdmin[0][1][address(mem[calldata.size + 96])].field_0:
        name[] = Array(len=arg1.length, data=arg1[all])
        domainSeperator = sha3(sha3(0x73454950373132446f6d61696e28737472696e67206e616d652c737472696e672076657273696f6e2c6164647265737320766572696679696e67436f6e74726163742c627974657333322073616c74), sha3(arg1[all]), sha3('1'), address(this.address), chainid)
