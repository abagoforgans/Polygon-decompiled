...
function withdrawBatch(uint256[] arg1) payable {
    ...
    while idx < arg1.length:
        ...
        continue 
    if this.address != msg.sender:
        emit WithdrawnBatch(Array(len=arg1.length, data=call.data[arg1 + 36 len floor32(arg1.length)]), msg.sender);
    else:
        _254 = mem[64]
        mem[64] = mem[64] + ceil32(calldata.size) + 32
        mem[_254] = calldata.size
        mem[_254 + 32 len calldata.size] = call.data[0 len calldata.size]
        emit WithdrawnBatch(Array(len=arg1.length, data=call.data[arg1 + 36 len floor32(arg1.length)]), mem[_254 + calldata.size + 12 len 20]);
}
